...
const ConfettiBurst: React.FC<Props> = ({
  ...
  const launch = () => {
    ...
    const baseCount = Math.round(count * (cssW < 480 ? 0.7 : 1));
    const N = baseCount * 2;
    ...
    const draw = () => {
      ...
      for (const p of parts) {
        ...
        ctx.rotate(p.r);
        if (shadow) { ctx.shadowColor = "rgba(0,0,0,.15)"; ctx.shadowBlur = 6; }
        ctx.fillStyle = p.c;
  ...
  // Resize con DPR (sin redibujar aquí)
  useEffect(() => {
    const canvas = canvasRef.current;
  ...
  // ⬇️ Espera a window.load y recién ahí lanza (luego respeta intervalMs)
  useEffect(() => {
    ...
    if (document.readyState === "complete") {
      ...
      const onLoad = () => { start(); window.removeEventListener("load", onLoad); };
