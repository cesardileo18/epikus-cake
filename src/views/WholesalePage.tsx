...
const WholesalePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#ff7bab48] pt-14">
      {/* ── PRODUCTOS POR CATEGORÍA ── */}
      {categories.map((category) => {
        const categoryProducts = products.filter((p) => p.category_id === category.id);

        return (
          <section key={category.id} className="max-w-7xl mx-auto px-4 py-4">
            {/* Section header */}
            <div className="text-center mb-4">
              <h2 className="text-3xl md:text-4xl font-light text-gray-900 mb-2 pt-4">
              ...
              <p className="text-sm pb-4">{category.subtitle}</p>
            </div>
      ...
      {/* ── CTA WHATSAPP ── */}
      <section className="bg-gradient-to-br from-pink-100/60 to-rose-100/40 py-16 px-6 text-center">
        <h2 className="text-2xl md:text-3xl font-light text-gray-900 mb-3">
          {page.cta.title}
        </h2>
        <p className="mb-8 text-sm">{page.cta.subtitle}</p>
        <a
          href={`https://wa.me/${page.cta.whatsapp_number}?text=${encodeURIComponent(page.cta.whatsapp_message)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-3 bg-green-500 hover:bg-green-600 text-white font-bold px-8 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5"
        >
