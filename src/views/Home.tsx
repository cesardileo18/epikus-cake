...
const Home: React.FC = () => {
  const [productosDestacados, setProductosDestacados] = useState<ProductWithId[]>([]);
  ...
  const cargarProductos = async (): Promise<void> => {
    try {
      ...
      const destacados = productosData.filter(producto =>
        producto.destacado && producto.activo
      ).slice(0, 6);
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>): void => {
    ...
    target.src = 'https://via.placeholder.com/400x400/f8fafc/64748b?text=Imagen+no+disponible';
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-rose-50">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-pink-500/10 via-rose-400/5 to-pink-600/10"></div>
  ...
  {features.map(({ title, icon: Icon, desc }) => (
    <div key={title} className="text-center">
      ...
      <p className="mt-1 text-sm text-gray-600">{desc}</p>
        ...
        <div className="absolute top-20 left-10 w-20 h-20 bg-gradient-to-r from-pink-400 to-rose-300 rounded-full opacity-20 animate-bounce"></div>
        <div className="absolute bottom-32 right-16 w-16 h-16 bg-gradient-to-r from-yellow-400 to-orange-300 rounded-full opacity-30 animate-pulse"></div>
        <div className="absolute top-1/3 right-8 w-12 h-12 bg-gradient-to-r from-purple-400 to-pink-300 rounded-full opacity-25 animate-bounce"></div>
      </section>
            ...
            {loading ? (
              ...
              <div className="col-span-full text-center py-16">
                ...
                <p className="text-gray-600 mb-8">
                  Estamos preparando deliciosas sorpresas para ti
                </p>
              </div>
            ) : (
              productosDestacados.map((producto: ProductWithId) => (
                <div key={producto.id} className="group relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2">
